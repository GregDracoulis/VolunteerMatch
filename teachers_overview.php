...
if ($_POST["doaction"] == "new_tar")
{
  ...
  if ($subject != "" AND $grades != "" AND $students != "" AND $category != "" AND sizeof($months) > 0 AND $bad_words == 0)
  {
    $month_array = Array();
    foreach ($months as $month)
    {
...
while (list($tid,$tsubject,$tgrades,$tstudents,$tmonths,$tcategory,$tbest_times,$tdetails,$tsubmit_date,$email_status) = $DB_site->fetch_array($tquery))
{
  ...
  $current_requests.= "<tr class=\"trclass$current_color\">
                        <td class=\"results\">$tmonths</td>
                        <td class=\"results\">$tsubmit_date</td>
                        <td class=\"results\">$category_name</td>
                        <td class=\"results\">$tsubject</td>
                        <td class=\"results\">$tstudents</td>
                        <td class=\"results\">$tbest_times</td>
                        <td class=\"results\">$tdetails</td>
	...
	($email_status == "Scheduled") ? $current_requests.= "" : $current_requests.= "<a href=\"$website_teachers_overview?doaction=modify_email_status&email_status=Scheduled&tid=$tid\">Scheduled</a> ";

	$current_requests.="</td>
                        <td class=\"results\"><a href=\"$website_teachers_overview?doaction=modify&tid=$tid\">Modify</a></td>
                        <td class=\"results\"><a href=\"$website_teachers_overview?doaction=delete&tid=$tid\">Delete</a></td>
                        <td class=\"results\"><a href=\"$website_teachers_overview?doaction=complete&tid=$tid\">Mark as Completed</a></td>
                      </tr>";
  $request_count++;
...
$current_color = 0;
$completed_requests = "";
$tquery = $DB_site->query("SELECT id,subject,grades,students,months,category,best_times,details,DATE(submit_time),volunteer,DATE(complete_time),rating FROM tars WHERE teacher_id='$member_id' AND tar_status='Complete' ORDER BY ID DESC LIMIT 5");
while (list($tid,$tsubject,$tgrades,$tstudents,$tmonths,$tcategory,$tbest_times,$tdetails,$tsubmit_date,$volunteer,$complete_date,$rating) = $DB_site->fetch_array($tquery))
{
  ...
  $completed_requests.= "<tr class=\"trclass$current_color\">
                        ...
                        <td class=\"results\">$tsubject</td>
                        <td class=\"results\">$tbest_times</td>
                        <td colspan='4'  class=\"results\">$tdetails</td>
                        <td class=\"results\">$volunteer_name ($volunteer_email)</td>
...
if ($completed_request_count == 0)
  $completed_requests.= "<tr><td colspan=\"14\" align=\"center\">No Completed Requests</td></tr>";
